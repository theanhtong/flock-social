...
-- ============================================================
-- Auto-expire suspensions once expires_at has passed.
-- Triggers only fire on writes, not on the passage of time, so
-- this needs a scheduled job (pg_cron) to periodically flip
-- expired suspensions to status = 'expired'. That UPDATE then
-- fires trg_sync_user_status_on_sanction_change above, which
-- resets users.status back to 'active' automatically.
-- If pg_cron isn't available on your Postgres host, run this
-- exact UPDATE from an external scheduler (e.g. BullMQ cron)
-- instead — the sync logic itself still lives entirely in the
-- trigger, the external job just needs to "wake it up".
-- ============================================================
CREATE EXTENSION IF NOT EXISTS pg_cron;

SELECT cron.schedule(
  'expire-user-sanctions',
  '*/5 * * * *', -- every 5 minutes, adjust as needed
  $$
    UPDATE user_sanctions
    SET status = 'expired'
    WHERE status = 'active' AND type = 'suspension'
